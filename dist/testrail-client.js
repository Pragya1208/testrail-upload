...
export class TestRailClient {
     ...
     */
    async resolveFrameworkFieldNameAndOptions() {
    ...
    buildCasePayload(row, options, templateId) {
            ...
            if (options.priorityNameToId && Object.keys(options.priorityNameToId).length > 0) {
                priorityId =
                    (pNum ? options.priorityNameToId[pNum] : undefined) ??
                        ...
                        Object.entries(options.priorityNameToId).find(([k]) => k.toLowerCase() === semanticKey)?.[1];
            }
            payload.priority_id = (priorityId !== undefined ? priorityId : priorityToId(priorityLabel));
        }
        if (row.estimate) {
        ...
        const frameworkLabel = framework && FRAMEWORK_VALUES.includes(framework)
            ? framework
        ...
        if (frameworkLabel) {
            ...
            let frameworkValue = frameworkLabel;
            if (options.frameworkLabelToId && Object.keys(options.frameworkLabelToId).length > 0) {
                ...
                const idByLower = idByExact ??
                    Object.entries(options.frameworkLabelToId).find(([k]) => k.toLowerCase() === frameworkLabel.toLowerCase())?.[1];
                if (idByExact !== undefined || idByLower !== undefined) {
                    frameworkValue = idByExact ?? idByLower;
                }
            }
            payload[frameworkFieldKey] = frameworkValue;
        }
        ...
        if (row.type || options.defaultType) {
            ...
            const typeId = options.typeNameToId && Object.keys(options.typeNameToId).length > 0
                ? options.typeNameToId[typeKey] ??
                    Object.entries(options.typeNameToId).find(([k]) => k.toLowerCase() === typeKey)?.[1]
                : undefined;
